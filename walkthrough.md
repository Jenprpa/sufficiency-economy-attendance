...
## V2.2 Sprint 1 - Sufficiency Activity Planner Framework (Framework 2-3-4-3-4)

### 16. UI Renaming Verification
- [ ] Verify that all menu links, sidebar labels, page headers, modals, dialog titles, lists, confirmation popups, and detail pages have been updated from "Lesson Planner" / "แผนการจัดกิจกรรมการเรียนรู้" to **"แผนกิจกรรมพอเพียง"**.
- [ ] Verify that no old "ปฏิทินรายวิชา" menu remains visible in the sidebar navigation or headers.

### 17. Framework 2-3-4-3-4 Checkbox Selection Form
- [ ] Log in as a **Teacher** or **Admin**, go to **"แผนกิจกรรมพอเพียง"**, and click **"เขียนแผนใหม่"** or click edit on an existing plan.
- [ ] Verify that the form displays the **"กรอบแนวคิดเศรษฐกิจพอเพียง (Framework 2-3-4-3-4)"** checkbox section grouped into:
  - **เงื่อนไข 2** (Knowledge, Morality)
  - **หลักการ 3** (Moderation, Reasonableness, Self-Immunity)
  - **มิติ 4** (Economic, Social, Environmental, Cultural)
  - **ศาสตร์ 3** (Royalศาสตร์, Science & Technology, Local Wisdom)
  - **พระราโชบาย 4** (ทัศนคติที่ถูกต้องต่อบ้านเมือง, พื้นฐานชีวิตที่มั่นคง - มีคุณธรรม, มีงานทำ - มีอาชีพ, เป็นพลเมืองที่ดี)
- [ ] Verify that checkbox cards toggle highlight state on select and are styled beautifully with dynamic background/border colors (HSL theme).

### 18. Saving & Firestore Schema Serialization
- [ ] Fill in details, check multiple options in the framework grid, and click **"บันทึกแบบร่าง"** or submit.
- [ ] Verify that the plan saves successfully without any console errors.
- [ ] Verify that the Firestore document (or local storage entry) contains a structured `framework` object containing the selected string arrays:
  - `conditions`
  - `principles`
  - `dimensions`
  - `sciences`
...
### 19. Detailed View Framework Pill Badges
- [ ] Click **"ดูรายละเอียด"** on any plan.
- [ ] Verify that selected framework options are displayed as colored pill badges grouped under their categories (เงื่อนไข 2, หลักการ 3, มิติ 4, ศาสตร์ 3, พระราโชบาย 4).
- [ ] Verify that empty categories (where no options are selected) are not rendered to keep the UI clean.

### 20. Backward Compatibility (Auto-Initialization)
- [ ] Load an older plan created in V2.0.1/V2.1.0 that does not have the `framework` key.
- [ ] Verify that the detailed view and edit form load successfully without crashing, automatically initializing missing framework arrays to empty arrays `[]`.
